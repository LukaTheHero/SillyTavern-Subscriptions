...
export async function geminiStatus() {
    ...
    let message;
    if (!launch && !api) message = 'Antigravity CLI (agy) not found and no Gemini/LinkAPI key set — install agy (https://antigravity.google/cli) and run it once to sign in, or set LINKAPI_ANTIGRAVITY_API_KEY.';
    return {
        provider: 'gemini',
        ok: available,
        available,
