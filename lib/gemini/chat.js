...
//                          is signed in to).
// backend = api          → direct OpenAI-compatible HTTP with GEMINI_API_KEY
//                          to GOOGLE_GEMINI_BASE_URL (default: Google's
...
export function discoverGeminiApiCredentials(req) {
    ...
    const key = pick(fromHeader, process.env.ST_SUBSCRIPTIONS_GEMINI_API_KEY, process.env.GEMINI_API_KEY, process.env.GOOGLE_API_KEY, settings?.env?.GEMINI_API_KEY);
    if (!key) return null;
    ...
    const baseUrl = pick(process.env.ST_SUBSCRIPTIONS_GEMINI_BASE_URL, process.env.GOOGLE_GEMINI_BASE_URL, settings?.env?.GOOGLE_GEMINI_BASE_URL) || GOOGLE_OPENAI_COMPAT_BASE;
...
export async function runGeminiChat({ req, messages, model, settings, writer }) {
    ...
    try {
        ...
        if (backend === 'api') {
            if (!creds) throw Object.assign(new Error('Gemini backend "api" selected but no key found — set GEMINI_API_KEY (+ optional GOOGLE_GEMINI_BASE_URL for a compatible relay) or paste the key into SillyTavern\'s Custom API key field.'), { httpStatus: 400 });
