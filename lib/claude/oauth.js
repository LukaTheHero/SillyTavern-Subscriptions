// ──────────────────────────────────────────────
// OAuth credential refresh + quota + rate-limit capture + error classification
// ──────────────────────────────────────────────
//
...
// queries start failing with auth errors. The fix (from Meridian) is an
// out-of-band refresh against the platform token endpoint followed by ONE
// retry of the query. Same client_id the CLI itself uses.
//
// Quota: two sources, cheapest first.
//   1. Every SDK query streams `rate_limit_event` messages carrying the
//      five-hour / seven-day utilisation ("unifiedWindows") — captured here
//      for free as chats happen.
...
export function isRateLimitError(text) {
    ...
    return s.includes('429') || s.includes('rate limit') || s.includes('too many requests')
        || s.includes("you've hit your limit") || s.includes('usage limit') || s.includes('hit your limit');
}
...
/** Fable's safeguard classifier refused the message ("safeguards flagged this message"). */
export function isSafeguardError(text) {
    const s = String(text ?? '').toLowerCase();
    return s.includes('safeguards flagged') || (s.includes("can't respond to this message") && s.includes('safeguard'));
}
...
async function doRefresh() {
    ...
    if (!refreshToken) {
        console.warn(`${TAG} token refresh skipped: no refresh token at ${path}`);
        return false;
    }
    try {
        ...
        if (!res.ok) {
            console.warn(`${TAG} token refresh failed: HTTP ${res.status}. Run \`claude login\` on this host.`);
            return false;
        }
        const data = await res.json();
        const now = Date.now();
        // Refresh tokens ROTATE and the CLI manages the same file — re-read
        // before writing; if another process already rotated, keep its copy.
...
export function recordRateLimitEvent(info) {
    ...
    for (const [key, w] of Object.entries(uw)) {
        if (!w) continue;
        const utilization = typeof w.utilization === 'number' ? Math.max(0, w.utilization) : null;
        const resetsAt = typeof w.resetsAt === 'number' ? (w.resetsAt < 1e12 ? w.resetsAt * 1000 : w.resetsAt) : null;
...
const WINDOW_TYPES = ['five_hour', 'seven_day', 'seven_day_opus', 'seven_day_sonnet', 'seven_day_fable', 'seven_day_oauth_apps'];
...
export async function fetchQuota({ force = false } = {}) {
    ...
    const extraUsage = extra
        ? { isEnabled: !!extra.is_enabled, monthlyLimit: extra.monthly_limit ?? 0, usedCredits: extra.used_credits ?? 0, currency: extra.currency ?? 'USD' }
        : null;
...
export async function claudeQuota() {
    ...
    return { ok: false, message: 'Quota unavailable (no OAuth credentials or upstream error).' };
}
