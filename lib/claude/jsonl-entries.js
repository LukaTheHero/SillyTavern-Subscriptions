...
import { buildPrefillContinuation } from '../common/messages.js';

const PLUGIN_TAG = '[subscriptions/claude]';
...
// Prefill continuation wording lives in common/messages.js (shared by every provider;
// phrasing verified against Fable's safeguard classifier on the resume path).
export { buildPrefillContinuation as buildAssistantPrefillContinuationPrompt };
...
export function splitHistoryForResume(messages) {
    ...
    if (trailing.role === 'assistant') {
        return {
            ...
            current: { role: 'user', content: buildPrefillContinuation(textFromContent(trailing.content)) },
...
import { detectSdkVersion } from './sdk-loader.js';
