...
// ──────────────────────────────────────────────
//
// Two-layer model story (Meridian's mechanism):
//   1. The picker exposes full model IDs plus explicit "(1M context)" variants
//      suffixed `[1m]` — e.g. `claude-opus-5[1m]`.
//   2. A base request passes the full model ID straight to the SDK. A `[1m]`
//      request passes the CLI's tier alias with the 1M suffix (`opus[1m]`,
//      `fable[1m]`, `sonnet[1m]`) as Options.model and pins the tier's
//      concrete version via ANTHROPIC_DEFAULT_<TIER>_MODEL in the subprocess
//      env — exactly how the CLI's own "/model … [1m]" entries resolve.
//
// 1M context needs Extra Usage on some plans; failures fall back to the base
// model with a one-hour probe cooldown (see chat.js).
...
// bundled (possibly stale) defaults never decide a tier we didn't pin.
...
// `adaptiveOnly` = always-thinking families; sampling params are rejected and
// `thinking: { type: 'enabled' | 'disabled' }` is invalid — only adaptive
// applies (Opus 4.7+, Opus 5, Fable, Mythos).
...
export function parseClaudeModel(requested) {
    ...
    return {
        ...
        context: entry ? entry.context : 200000,
    };
}
...
/** OpenAI-style model list entries (base + 1M variants). */
export function listClaudeModels() {
    const data = [];
    for (const m of CLAUDE_MODELS) {
        data.push({ id: m.id, object: 'model', created: 0, owned_by: 'anthropic', display_name: m.name, context_window: m.context, provider: 'claude' });
        if (m.oneM) {
            data.push({ id: `${m.id}${ONE_M_SUFFIX}`, object: 'model', created: 0, owned_by: 'anthropic', display_name: `${m.name} (1M context)`, context_window: 1000000, provider: 'claude' });
        }
    }
    return data;
}
