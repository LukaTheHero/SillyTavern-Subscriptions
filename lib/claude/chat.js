...
/** Served-model guard: never silently substitute another model for an explicit Fable request. */
        ...
        while (true) {
            ...
            try {
                ...
                if (!writer.didYieldContent) {
                    ...
                    // Fable's safeguard classifier sometimes rejects a resumed
                    // session whose final turn is a prefill-continuation
                    // instruction (verified: the same chat passes when folded).
                    if (isSafeguardError(errText) && cfg.path === 'resume') {
                        console.warn(`${TAG} safeguard rejection on the resume path — retrying via transcript fold`);
                        settings.claude.useResume = false;
                        continue;
                    }
                }
