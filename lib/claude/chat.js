...
//      cooldown; rate limit → 2 retries; quota exhausted → API-key
...
function friendlyHint(text) {
    ...
    if (isQuotaExhaustedError(text)) return ' — subscription window exhausted; wait for the reset, or set Claude backend to "api" with an API key for pay-per-token overflow.';
    return '';
}
