// ──────────────────────────────────────────────
// Claude backend selection: subscription vs API key / LinkAPI overflow
// ──────────────────────────────────────────────
//
// backend = subscription  → OAuth login (claude login), bills the Pro/Max plan.
// backend = api           → ANTHROPIC_BASE_URL + key/token. Key sources, in
//                           order: the Custom API key field in SillyTavern
//                           (any non-placeholder value), ST_SUBSCRIPTIONS_
//                           CLAUDE_API_KEY, LINKAPI_CLAUDE_API_KEY, the
//                           process env, ~/.claude/settings.json's env block
//                           (what the LinkAPI overflow toggle writes).
...
export function discoverApiCredentials(req) {
    ...
    let key = pick(fromHeader, process.env.ST_SUBSCRIPTIONS_CLAUDE_API_KEY, process.env.LINKAPI_CLAUDE_API_KEY,
        process.env.ANTHROPIC_AUTH_TOKEN, process.env.ANTHROPIC_API_KEY, envBlock.ANTHROPIC_AUTH_TOKEN, envBlock.ANTHROPIC_API_KEY);
    if (!key) return null;
    ...
    else if (key === process.env.LINKAPI_CLAUDE_API_KEY) source = 'LINKAPI_CLAUDE_API_KEY';
    else if (key === process.env.ANTHROPIC_AUTH_TOKEN || key === process.env.ANTHROPIC_API_KEY) source = 'process-env';
    else source = 'claude-settings.json';

    const isLinkApi = source === 'LINKAPI_CLAUDE_API_KEY';
    let baseUrl = pick(process.env.ST_SUBSCRIPTIONS_CLAUDE_BASE_URL, process.env.ANTHROPIC_BASE_URL, envBlock.ANTHROPIC_BASE_URL);
    // A LinkAPI relay key never belongs to api.anthropic.com — a stray
    // ANTHROPIC_BASE_URL pointing there (e.g. inherited from a Claude Code
    // terminal) must not pair with it.
    if (isLinkApi && (!baseUrl || /anthropic\.com/i.test(baseUrl)) && !process.env.ST_SUBSCRIPTIONS_CLAUDE_BASE_URL) baseUrl = LINKAPI_DEFAULT_BASE;
    if (!baseUrl && isLinkApi) baseUrl = LINKAPI_DEFAULT_BASE;
    // A real Anthropic key with no base URL goes straight to Anthropic; any other
    // key without a base URL is almost certainly a LinkAPI relay key.
    if (!baseUrl && !isAnthropicApiKey(key)) baseUrl = LINKAPI_DEFAULT_BASE;

    return {
        mode: 'api',
        baseUrl: baseUrl || undefined,
        // sk-ant-* → x-api-key auth; relay keys → bearer token (what the LinkAPI toggle uses).
...
export function chooseClaudeAuth(backend, req) {
    ...
    if (backend === 'api') {
        if (!api) {
            const err = new Error(
                'Claude backend "api" selected but no API key/LinkAPI key was found. Put the key in SillyTavern\'s ' +
                'Custom API key field, or set ST_SUBSCRIPTIONS_CLAUDE_API_KEY / LINKAPI_CLAUDE_API_KEY (+ optional ' +
                'ST_SUBSCRIPTIONS_CLAUDE_BASE_URL) in SillyTavern\'s environment.',
            );
            err.httpStatus = 400;
            throw err;
        }
