...
//                          direct HTTP when no CLI exists but a key does.
...
export function discoverCodexApiCredentials(req) {
    ...
    const key = pick(fromHeader, process.env.ST_SUBSCRIPTIONS_CODEX_API_KEY, process.env.OPENAI_API_KEY);
    if (!key) return null;
    ...
    try {
        ...
        if (cliAvailable) {
            ...
            if (!auth.present && !usesOverflow) {
                if (creds) return await runViaDirectApi({ messages, model, settings, writer, creds, signal: ac.signal });
                throw Object.assign(new Error(`Codex CLI is installed but not logged in (no auth.json in ${auth.path.replace(/[\\/]auth\.json$/, '')}) and no API key is set. Run \`codex login\`, or set OPENAI_API_KEY.`), { httpStatus: 400 });
            }
